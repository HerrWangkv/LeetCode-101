...
```
- 确定无环链表索引为`(n-1)//2`的中间节点：见[middle of the linked list](submissions/876.middle-of-the-linked-list.cpp)
...
# 排序
## 归并排序 merge sort
divide and conquer。将整个链表/列表递归地二分，处理再融合。对链表归并排序需要使用快慢指针来分半，对于列表归并排序需要。
...
## 单调栈
单调栈中的“单调”指的是以栈中元素为索引的元素符合单调排列。每个元素的索引都会入栈，但要保证单调性。
